...
class GUI():
    def __init__(self) -> None:
        ...
        with ui.list():                      ### make updates to this reflect in code
            ...
            for col in cols:
                colName, _ = col.split()
                checkbox = ui.checkbox(colName).props(f"id={colName}-checkbox")
        ...
        ui.markdown("#### Launch")
        ui.button("Start!", on_click=lambda: self.runML.refresh(type=0)).props("id=start-button")
    ...
    def runML(self, type=0):
        ...
        ui.markdown(f"> The best performing algorithm was: {best.capitalize()}    \n\
                        ...
                        It had a MAPE of {results[best][2]:.5f}%").props('id=results')
        for algo in results:
            if algo is not best:
